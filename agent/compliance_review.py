"""
agent/compliance_review.py - Governance and compliance validation pipeline.

Independent verification suite auditing drafted responses prior to client dispatch:
1. Legal & Regulatory Compliance: Validates currency formatting, non-discrimination, and statutory rules.
2. Data Privacy & Security: Confirms complete PII redaction and checks refusal of adversarial prompts.
3. Knowledge Groundedness & QA: Verifies policy citations against the official document index.
"""
...
class LegalComplianceOfficer(ComplianceReviewer):
    def __init__(self):
        super().__init__(
            name="Meera Sen (Legal & Employment Compliance)",
...
class PrivacySecurityOfficer(ComplianceReviewer):
    def __init__(self):
        super().__init__(
            name="Arjun Varma (InfoSec & Data Privacy)",
            role="Data Protection & AI Security Lead",
        )
...
class GroundednessAuditor(ComplianceReviewer):
    def __init__(self):
        super().__init__(
            name="Rajeshwari Nair (Knowledge QA & Grounding)",
            role="Quality & Groundedness Auditor",
        )
